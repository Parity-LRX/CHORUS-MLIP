...
Usage:
    # Export:
    python -m chorus.cli.export_mliap checkpoint.pth --elements H O
...
try:
    from chorus.models import E3_TransformerLayer_multi
except Exception:
    E3_TransformerLayer_multi = None
try:
    from chorus.models.e3nn_layers_channelwise import (
        E3_TransformerLayer_multi as E3_TransformerLayer_multi_channelwise,
    )
except Exception:
    E3_TransformerLayer_multi_channelwise = None
try:
    from chorus.models.pure_cartesian_sparse_layers import (
        PureCartesianSparseTransformerLayer,
    )
except Exception:
    PureCartesianSparseTransformerLayer = None
try:
    from chorus.models.pure_cartesian_sparse_layers_save import (
        PureCartesianSparseTransformerLayerSave,
    )
except Exception:
    PureCartesianSparseTransformerLayerSave = None
try:
    from chorus.models.pure_cartesian_ictd_layers_full import PureCartesianICTDTransformerLayer
except Exception:
    PureCartesianICTDTransformerLayer = None
try:
    from chorus.models.pure_cartesian_ictd_layers_full_o3 import PureCartesianICTDO3TransformerLayer
except Exception:
    PureCartesianICTDO3TransformerLayer = None
try:
    from chorus.models.pure_cartesian_ictd_layers_o3 import PureCartesianICTDSaveO3TransformerLayer
except Exception:
...
from chorus.utils.external_tensor_specs import normalize_external_tensor_specs
from chorus.models.zbl import maybe_wrap_model_with_zbl
...
def _warm_ictd_projector_caches_for_trace(model: nn.Module) -> None:
    """Build ICTD CG projector caches before TorchScript tracing.

    If the cache is cold, tracing records the Python projector construction
    graph itself.  That construction includes temporary tensor reshapes that
    are unrelated to the input graph but can replay incorrectly when LAMMPS
    later calls the traced core with a different atom/edge count.  Warming the
    cache makes the projector tensors enter the traced graph as constants.
    """
    for module in model.modules():
        getter = getattr(module, "_get_proj_group_list", None)
        ...
        dtype = ref.dtype if ref is not None and ref.is_floating_point() else torch.get_default_dtype()
        try:
            getter(device=device, dtype=dtype)
        except Exception:
            # Cache warming is an export robustness optimization; leave the
            # original model error path intact if an unusual module rejects it.
            continue
...
class _TorchScriptEdgeVecCore(nn.Module):
    ...
    def __init__(self, model: nn.Module, *, export_reciprocal_source: bool = False):
        super().__init__()
        _warm_ictd_projector_caches_for_trace(model)
        self.model = model
    ...
    def from_checkpoint(
        ...
        if mode == "pure-cartesian-ictd-fix":
            ...
            if has_softmax_heads:
                ...
                inferred_head_mode = "softmax"
            # Reconstruct optional invariant neighbor attention. Legacy checkpoints
            # expose attn_z_bias_raw in every active layer; density-preserving
            # checkpoints omit that null-logit parameter and may activate only the
            # final interaction.
            attn_layer_indices = sorted(
                {
                    int(key.split(".")[1])
                    for key in selected_state_dict
                    if key.startswith("interactions.")
                    and key.endswith(".attn_logit_w")
                }
            )
            inferred_attn_state_heads = next(
                (
                    int(value.shape[0])
            ...
            inferred_attn_heads = int(
                ...
                or inferred_attn_state_heads
            )
            inferred_attn_mode = str(
                ckpt.get("ictd_fix_interaction_attn_mode")
                or arch_meta.get("ictd_fix_interaction_attn_mode")
                or (
                    "legacy-softmax"
                    if any(
                        key.endswith(".attn_z_bias_raw")
                        for key in selected_state_dict
                    )
                    else (
                        "density-preserving"
                        if inferred_attn_heads > 0
                        else "legacy-softmax"
                    )
                )
            )
            inferred_attn_scope = str(
                ckpt.get("ictd_fix_interaction_attn_scope")
                or arch_meta.get("ictd_fix_interaction_attn_scope")
                or (
                    "final"
                    if (
                        inferred_attn_heads > 0
                        and len(attn_layer_indices) == 1
                        and attn_layer_indices[0] == int(num_interaction) - 1
                        and int(num_interaction) > 1
                    )
                    else "all"
                )
            )
            ...
            inferred_phase_coefficient = str(
                ckpt.get("ictd_fix_phase_coefficient")
                or arch_meta.get("ictd_fix_phase_coefficient", "polar")
            )
            inferred_phase_context = str(
                ckpt.get("ictd_fix_phase_context")
                or arch_meta.get("ictd_fix_phase_context", "content")
            )
            inferred_phase_density_pairs = str(
                ckpt.get("ictd_fix_phase_density_pairs")
                or arch_meta.get("ictd_fix_phase_density_pairs")
                or (
                    "full-adaptive"
                    if any(
                        key.endswith(".coherence_logit")
                        for key in selected_state_dict
                    )
                    else "full-gated"
                    if any(
                        key.endswith(".coherence_scale")
                        for key in selected_state_dict
                    )
                    else "full"
                )
            )
            inferred_phase_coherence_init = float(
                ckpt.get("ictd_fix_phase_coherence_init")
                or arch_meta.get("ictd_fix_phase_coherence_init", 0.1)
            )
            inferred_phase_normalization = str(
                ckpt.get("ictd_fix_phase_normalization")
                or arch_meta.get(
            ...
            inferred_phase_heads = int(
                ckpt.get("ictd_fix_phase_heads")
                or arch_meta.get("ictd_fix_phase_heads", 1)
            )
            ...
            model = PureCartesianICTDFix(
                ...
                ictd_fix_nonlinear_layer_readouts=bool(
                    ckpt.get(
                        "ictd_fix_nonlinear_layer_readouts",
                        arch_meta.get(
                            "ictd_fix_nonlinear_layer_readouts", False
                        ),
                    )
                ),
                ictd_fix_final_layer_readout_only=bool(
                    ckpt.get(
                        "ictd_fix_final_layer_readout_only",
                        arch_meta.get(
                            "ictd_fix_final_layer_readout_only", False
                        ),
                    )
                ),
                ictd_fix_element_energy_correction=bool(
                    ckpt.get(
                        "ictd_fix_element_energy_correction",
                        arch_meta.get(
                            "ictd_fix_element_energy_correction", False
                        ),
                    )
                ),
                ictd_fix_scalar_ffn=bool(
                    ckpt.get(
                        "ictd_fix_scalar_ffn",
                        arch_meta.get("ictd_fix_scalar_ffn", False),
                    )
                ),
                ictd_fix_edge_lmax=(
                    None
                ...
                ictd_fix_interaction_attn_mode=inferred_attn_mode,
                ictd_fix_interaction_attn_scope=inferred_attn_scope,
                ...
                ictd_fix_phase_coefficient=inferred_phase_coefficient,
                ...
                ictd_fix_phase_heads=inferred_phase_heads,
            ...
            from chorus.models.cue_layers_channelwise import (
