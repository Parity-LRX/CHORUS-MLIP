...
:class:`chorus.models.pure_cartesian_ictd_fix.PureCartesianICTDFix`. The exact
...
from chorus.models.ictd_irreps import direction_harmonics_all
...
def convert_mace_to_ictd(mace_model, ictd_model) -> Dict[str, object]:
    ...
    final_layer_readout_only = bool(
        getattr(ictd_model, "ictd_fix_final_layer_readout_only", False)
    )
    expected_intermediate_readouts = (
        0 if final_layer_readout_only else int(ictd_model.num_interaction) - 1
    )
    if len(ictd_model.layer_energy_readouts) != expected_intermediate_readouts:
        raise RuntimeError(
            "ICTC intermediate readout count does not match its readout topology"
        )

    if final_layer_readout_only:
        for ridx in range(len(mace_model.readouts) - 1):
            report["blocks"][f"readout[{ridx}]"] = (
                "MACE intermediate readout intentionally omitted by "
                "final-layer-only ICTC topology"
            )
    ...
    for ridx, (m_readout, i_readout) in enumerate(
        ...
        if hasattr(i_readout, "readout"):
            with torch.no_grad():
                i_readout.readout.weight.copy_(
                    ro_mats[0].T.to(dtype=i_readout.readout.weight.dtype)
                )
                if i_readout.readout.bias is not None:
                    i_readout.readout.bias.zero_()
            report["blocks"][f"readout[{ridx}]"] = (
                "MACE LinearReadout l=0 effective-matrix -> nn.Linear(C,1)"
            )
        elif hasattr(i_readout, "linear_1") and hasattr(i_readout, "linear_2"):
            # The optional nonlinear intermediate head has no exact linear-MACE
            # counterpart.  Preserve its own MACE-style random initialization
            # while still converting every shared trunk parameter.
            report["blocks"][f"readout[{ridx}]"] = (
                "nonlinear ICTC intermediate readout retained at native "
                "MACE-style initialization"
            )
        else:
            raise NotImplementedError(
                "unsupported ICTC intermediate readout "
                f"{type(i_readout).__name__}"
            )
