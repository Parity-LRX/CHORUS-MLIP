...
Usage:
  python -m chorus.test.bench_ictd_fix_trainstep --check
  python -m chorus.test.bench_ictd_fix_trainstep --check --save-ref /tmp/ref.pt
  python -m chorus.test.bench_ictd_fix_trainstep --check --compare-ref /tmp/ref.pt
  python -m chorus.test.bench_ictd_fix_trainstep --bench --atoms 512 --profile
"""
...
def run_makefx(args) -> int:
    ...
    print(f"[makefx] device={device} dtype={dtype} atoms={args.atoms} edges={args.atoms*args.degree} "
          f"channels={args.channels} lmax={args.lmax} L={args.num_interaction} route={args.route} "
          f"attn_heads={args.attn_heads} attn_mode={args.attn_mode} "
          f"attn_scope={args.attn_scope} compile={do_compile}")
    if device.type == "cuda":
...
def run_paths(args) -> int:
    """Print the ICTC-U symmetric-contraction path counts (each U's last axis = the
    ...
    from chorus.models.ictd_irreps import ictd_u_matrix_so3
    lmax = args.lmax
    print(f"[paths] lmax={lmax}  ICTC-U symmetric-contraction path counts (U last-axis = #paths)")
    per_deg: dict = {}
...
def main() -> int:
    ...
    p.add_argument("--paths", action="store_true", help="print ICTC-U symmetric-contraction path counts per degree (no model build / no CUDA)")
