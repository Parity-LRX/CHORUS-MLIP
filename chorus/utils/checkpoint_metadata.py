...
DEFAULT_MODEL_ARCHITECTURE: dict[str, Any] = {
    ...
    "ictd_fix_scalar_ffn": False,
    "ictd_fix_interaction_attn_heads": 0,
    "ictd_fix_interaction_attn_mode": "legacy-softmax",
    "ictd_fix_interaction_attn_scope": "all",
    ...
    "feature_spectral_assignment": "pcs",
...
def validate_dispersion_deployment_graph_rule(
    *,
    ...
    from chorus.models.dispersion import dispersion_deployment_graph_rule
...
def validate_dispersion_training_graph_rule(
    *,
    ...
    from chorus.models.dispersion import dispersion_training_graph_rule
...
def validate_dispersion_train_deploy_graph_compatibility(
    *,
    ...
    from chorus.models.dispersion import dispersion_train_deploy_graph_compatibility
...
def get_checkpoint_e3_state_dict(
    ...
    *,
    ...
    New MACE-ICTC checkpoints may carry ``default_state_source`` to choose raw,
...
def resolve_model_architecture(
    ...
    *,
    ...
    resolved["ictd_fix_nonlinear_layer_readouts"] = bool(
        _resolve_value(
            overrides,
            checkpoint,
            arch_meta,
            "ictd_fix_nonlinear_layer_readouts",
            DEFAULT_MODEL_ARCHITECTURE["ictd_fix_nonlinear_layer_readouts"],
        )
    )
    resolved["ictd_fix_final_layer_readout_only"] = bool(
        _resolve_value(
            overrides,
            checkpoint,
            arch_meta,
            "ictd_fix_final_layer_readout_only",
            DEFAULT_MODEL_ARCHITECTURE["ictd_fix_final_layer_readout_only"],
        )
    )
    resolved["ictd_fix_element_energy_correction"] = bool(
        _resolve_value(
            overrides,
            checkpoint,
            arch_meta,
    ...
    resolved["ictd_fix_scalar_ffn"] = bool(
        _resolve_value(
            overrides,
            checkpoint,
            arch_meta,
            "ictd_fix_scalar_ffn",
            DEFAULT_MODEL_ARCHITECTURE["ictd_fix_scalar_ffn"],
        )
    )
    resolved["ictd_fix_interaction_attn_heads"] = int(
        _resolve_value(
            overrides,
            checkpoint,
            arch_meta,
            "ictd_fix_interaction_attn_heads",
            DEFAULT_MODEL_ARCHITECTURE["ictd_fix_interaction_attn_heads"],
        )
    )
    resolved["ictd_fix_interaction_attn_mode"] = str(
        _resolve_value(
            overrides,
            checkpoint,
            arch_meta,
            "ictd_fix_interaction_attn_mode",
            DEFAULT_MODEL_ARCHITECTURE["ictd_fix_interaction_attn_mode"],
        )
    )
    resolved["ictd_fix_interaction_attn_scope"] = str(
        _resolve_value(
            overrides,
            checkpoint,
            arch_meta,
            "ictd_fix_interaction_attn_scope",
            DEFAULT_MODEL_ARCHITECTURE["ictd_fix_interaction_attn_scope"],
        )
    )
    ...
    resolved["ictd_fix_phase_heads"] = int(
        _resolve_value(
            overrides,
            checkpoint,
            arch_meta,
            "ictd_fix_phase_heads",
            DEFAULT_MODEL_ARCHITECTURE["ictd_fix_phase_heads"],
        )
    )
