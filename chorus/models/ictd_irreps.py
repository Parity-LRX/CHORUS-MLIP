"""
ICTC/trace-chain based irreps (2l+1) utilities and tensor products WITHOUT spherical harmonics.
...
try:
    _triton_tp = __import__(
        "chorus.models.ictd_irreps_triton",
...
class MultipleContractionSO3(nn.Module):
    ...
    layout used by ICTC-save-multiple.
...
class HarmonicReconstructors:
    ...
    where ``c_l`` are harmonic coordinates in the canonical ICTC basis and
...
def build_harmonic_reconstructors(Lmax: int) -> HarmonicReconstructors:
    ...
    These matrices reconstruct monomial coefficients from ICTC harmonic coordinates.
    """
...
def _direction_component_scale_cpu_f64(l: int) -> float:
    """
    Scale ICTC direction harmonics to e3nn's component normalization.

    The ICTC harmonic basis is orthonormal under isotropic Gaussian polynomial
...
def _direction_harmonics_triton_optional(
    ...
    if os.environ.get("ICTD_USE_TRITON", "0") != "1":
        return None
    try:
        from chorus.models.ictd_irreps_triton import direction_harmonics_triton
...
def direction_harmonics_irreps(n: torch.Tensor, irreps: str) -> torch.Tensor:
    """
    Like e3nn spherical_harmonics(irreps_out, x): compute direction harmonics in ICTC basis
...
def ictd_l2_to_rank2(c: torch.Tensor) -> torch.Tensor:
    """
    Convert ICTC l=2 (5D) harmonic coordinates to 3x3 symmetric traceless tensor.

    The ICTC basis is built from monomials (x^a y^b z^c) with a+b+c=2.
    Monomial order: z^2, yz, y^2, xz, xy, x^2 (from _counts_list).
    Output T satisfies T(R·n) = R @ T(n) @ R.T under rotation R.

    Args:
        c: (..., 5) ICTC l=2 coordinates
    Returns:
...
def _harmonic_rotation_generator_cpu_f64(l: int, axis: str) -> torch.Tensor:
    """Infinitesimal SO(3) generator in the ICTC harmonic basis."""
...
def build_full_cg_tensor_so3(l1: int, l2: int, l3: int) -> torch.Tensor:
    """
    Full SO(3) Clebsch-Gordan tensor in the ICTC harmonic basis.
    ...
    solving the infinitesimal equivariance equations in the ICTC basis, without
...
def cg_tensor_sparsity(C: torch.Tensor, threshold: float = 1e-10) -> Tuple[int, int, float]:
    """
    Return (numel, num_nonzero, zero_fraction) for an ICTC CG tensor.
...
def _ictd_so3_coupled_basis(
    *,
    ...
    """
    Build n-body SO(3) coupling basis tensors in the ICTC harmonic basis.
...
def _ictd_u_matrix_so3_compute(
    ...
    """Exact float64 build of the MACE-style symmetric-contraction U from ICTC CG."""
    dtype = torch.float64
...
def ictd_u_matrix_so3(
    *,
    ...
    """MACE-style symmetric-contraction U matrix generated purely from ICTC CG tensors.
...
class HarmonicElementwiseProduct(nn.Module):
    """
    Element-wise tensor product in ICTC basis, analogous to e3nn ElementwiseTensorProduct.
...
class HarmonicFullyConnectedTensorProduct(nn.Module):
    """
    Fully-connected tensor product in harmonic/ICTC basis (SO(3) irreps, no spherical harmonics).
        ...
        # Still uses the e3nn-like factorization (projection first, then channel mixing).
            ...
            for g_idx, g in enumerate(self._groups):
                ...
                if y.shape[-1] != k_total:
                    raise RuntimeError("ICTC TP projection produced wrong K_total")
...
class HarmonicPathWeightedTensorProduct(nn.Module):
    """
    Lightweight path-weighted tensor product in the ICTC SO(3) basis.
    ...
    - reuses the CG / projection cache already available in ICTC irreps
    - learns only one scalar weight per `(path, channel)`

    This matches the "few path weights + fixed coupling basis" idea used by
    MACE-style symmetric contractions while staying fully within the ICTC
    operator family.
    """
    ...
    def fold_cg_to_e3nn(self, q_blocks: List[torch.Tensor]) -> None:
        ...
        with C the per-path ICTC Clebsch-Gordan tensor of shape (2l1+1, 2l2+1, 2l3+1).
...
class EdgeWeightedPathTensorProduct(HarmonicPathWeightedTensorProduct):
    ...
    This keeps the ICTC SO(3) path basis and path-weighted channel scaling from
...
class EdgeWeightedPathPreservingTensorProduct(EdgeWeightedPathTensorProduct):
    """
    Path-preserving ICTC SO(3) tensor product for MACE-style interactions.
    ...
    that layout while using the same ICTC CG/projector tensors as the compressed
    variant.
    """
...
class HarmonicFullyConnectedTensorProductO3(nn.Module):
    """
    Parity-aware O(3) fully-connected tensor product in ICTC basis.

    Inputs / outputs are keyed by (l, parity_sign), where parity_sign is +1/-1.
    CG tensors are reused from the SO(3) ICTC basis; parity only filters valid paths.
    """
...
class MultipleContractionO3(nn.Module):
    ...
    layout used by parity-aware ICTC models.

    This mirrors MultipleContractionSO3 but operates on (l, parity) blocks and
    reuses the existing O(3) fully-connected ICTC tensor product.
    """
...
class HarmonicChannelWiseTensorProductO3(HarmonicFullyConnectedTensorProductO3):
    """
    O(3) parity-aware channel-wise ICTC tensor product specialized for convolution-style usage.
...
class HarmonicChannelWiseTensorProduct(HarmonicFullyConnectedTensorProduct):
    """
    Channel-wise ICTC tensor product specialized for convolution-style usage.
