...
Run (from the repo root):
    ...
    # or:  PYTHONPATH=. python3 chorus/test/test_mace_converter.py
or under pytest.
"""
...
def test_mace_converter_final_layer_only_readout():
    mace = build_mace(
        first_interaction="residual",
        num_interactions=2,
        max_ell=LMAX,
    ...
    report = convert_mace_to_ictd(mace, ictd)
    assert len(ictd.layer_energy_readouts) == 0
    assert "intentionally omitted" in report["blocks"]["readout[0]"]
