...
def _resolve_compile_dynamic(value: bool | None) -> bool:
    if value is not None:
        return bool(value)
    return _env_flag("MACE_ICTC_MAKEFX_DYNAMIC", True)


def _resolve_compile_backend(value: str | None) -> str:
    return str(value or os.environ.get("MACE_ICTC_MAKEFX_BACKEND", "inductor"))


def _leading_shape_signature(example_inputs: Sequence[torch.Tensor], *, dynamic: bool) -> tuple[int, ...]:
    tensors = [t for t in example_inputs if torch.is_tensor(t)]
    if dynamic:
        # Dynamic Inductor graphs are intended to absorb edge-count variation, so
        # cache by atom count only.  The arity is keyed separately by the caller.
        return (int(tensors[0].shape[0]),) if tensors else ()
    # Static Inductor graphs specialize every leading dimension; include all
    # tensor leading dims so a graph compiled for one edge count is never reused
    # for another.
    return tuple(int(t.shape[0]) for t in tensors)
...
def trace_and_compile_force(
    ...
    *,
    ...
    compile_dynamic_shapes: bool | None = None,
    ...
    try:
        ...
        # Warm up lazy device/dtype-keyed caches (CG tensors, projection groups, ICTC
    ...
    backend = _resolve_compile_backend(compile_backend)
    dynamic = _resolve_compile_dynamic(compile_dynamic_shapes)
    if backend == "inductor":
        opts = _filter_inductor_opts(inductor_options if inductor_options is not None else _INDUCTOR_OPTS)
        return torch.compile(traced, backend=backend, dynamic=dynamic, options=opts)
    return torch.compile(traced, backend=backend, dynamic=dynamic)
...
class CompiledForceCache:
    ...
    Two reasons for the key:
      ...
      * **the leading dim of every input** (atom count N, edge count E, ...): make_fx
        ...
        C, 2l+1)`` reshapes in the ICTC layers specialize on N), so a graph traced at
    ...
    def get(
        ...
        **compile_kwargs: Any,
        ...
        # Dynamic graphs are keyed by atom count only: they are intended to absorb
        # edge-count variation at runtime. Static graphs specialize every leading
        # dimension, so include all input leading dims to avoid wrong-shape reuse.
        dynamic = _resolve_compile_dynamic(compile_kwargs.get("compile_dynamic_shapes"))
        shape_key = _leading_shape_signature(example_inputs, dynamic=dynamic)
