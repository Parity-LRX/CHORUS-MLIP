"""Compact energy+force(+stress) trainer for MACE-ICTC (make_fx compile + size-bucketing).

This is the baseline energy+force training path extracted from the FSCETP trainer,
kept numerically faithful but stripped of the modes MACE-ICTC does not ship
...
from chorus.utils.checkpoint_metadata import get_checkpoint_e3_state_dict
from chorus.utils.scatter import scatter
from chorus.utils.tensor_utils import map_tensor_values
from chorus.utils.fidelity import smooth_l1_loss_stats
from chorus.models.losses import RMSELoss
...
class ForceTrainer:
    ...
    def __init__(
        ...
        *,
        ...
        phase_rank_orthogonal_weight: float = 0.0,
        ...
        self.phase_rank_orthogonal_weight = float(phase_rank_orthogonal_weight)
        if self.phase_rank_orthogonal_weight < 0.0:
            raise ValueError("phase_rank_orthogonal_weight must be non-negative")
        ...
        for name, param in self._raw_model.named_parameters():
            ...
            if name.startswith("node_embedding"):
                ...
                key = (
                    "other_no_decay"
                    if name.endswith(
                        ("residual_scale", "coherence_scale", "coherence_logit")
                    )
                    else "products"
                )
    ...
    # ----------------------------------------------------------------- make_fx
    def _complete_mbd_training_ramp_for_makefx(self) -> None:
        raw_model = self.model.module if hasattr(self.model, "module") else self.model
        for module in raw_model.modules():
            complete = getattr(module, "complete_training_ramp", None)
    ...
    def _makefx_forward(
        ...
        from chorus.training.makefx_compile import CompiledForceCache

        if self.model.training:
            self._complete_mbd_training_ramp_for_makefx()
    ...
    def _dispersion_edges_for_batch(self, pos, batch_idx, cell, extras):
        ...
        if disp_src is not None or disp_dst is not None or disp_shifts is not None:
            ...
            from chorus.models.dispersion import normalize_dispersion_edges
        ...
        from chorus.models.dispersion import dispersion_neighbor_list
    ...
    def _phase_rank_orthogonal_penalty(self) -> torch.Tensor:
        """Return a rank-normalized ||P P^T - I||_F^2 penalty.

        Only CHORUS full-L rank projections expose ``rank_projections``.  The
        division by the number of projected rows keeps the regularizer scale
        comparable when density rank changes.
        """
        penalties = []
        for module in self._raw_model.modules():
            projections = getattr(module, "rank_projections", None)
            if not isinstance(projections, torch.nn.ModuleDict):
                continue
            for projection in projections.values():
                ...
                penalties.append((gram - identity).square().sum() / gram.shape[0])
        if not penalties:
    ...
    def load_checkpoint(self, path, *, training_state: bool = False, strict: bool = True) -> int:
        ...
        if training_state:
            # Optimizer continuation must always resume the instantaneous/raw
            # parameters whose optimizer moments are stored in the checkpoint.
            state = ckpt.get("e3trans_state_dict", ckpt)
            state_source = "raw"
        elif isinstance(ckpt, dict) and "e3trans_state_dict" in ckpt:
            # Evaluation/deployment should honor the checkpoint's declared
            # default (EMA/SWA/raw), matching the non-training load paths.
            state, state_source = get_checkpoint_e3_state_dict(ckpt)
        else:
            state = ckpt
            state_source = "raw"
        ...
        if training_state:
            ...
            log.info("loaded model weights from %s (state_source=%s)", path, state_source)
        return start_epoch
    ...
    def fit_element_energy_correction_from_training_set(self) -> dict:
        """Exactly refit the coordinate-independent element energies on train.

        Joint energy/force optimization can leave these few directions poorly
        conditioned when the force term dominates.  This is one block-coordinate
        update of ``min_delta sum_s (e_s + x_s @ delta)^2``, where ``e_s`` is
        the current per-atom energy error and ``x_s`` contains element fractions.
        It uses training labels only and changes neither forces nor inference cost.
        """
        raw_model = self._raw_model
        correction = getattr(raw_model, "element_energy_correction", None)
        if not isinstance(correction, torch.nn.Parameter):
            raise RuntimeError(
                "closed-form element calibration requires a model built with "
                "element_energy_correction=True"
            )
        if self._default_state_source() != "raw":
            raise RuntimeError(
                "closed-form element calibration currently requires raw checkpoint "
                "weights; set --checkpoint-state-source raw and do not combine it "
                "with EMA/SWA"
            )
        atomic_number_to_index = getattr(
            raw_model, "atomic_number_to_index", None
        )
        if not torch.is_tensor(atomic_number_to_index):
            raise RuntimeError(
                "model does not expose atomic_number_to_index for element calibration"
            )

        num_elements = int(correction.numel())
        xtx = torch.zeros(
            (num_elements, num_elements),
            dtype=torch.float64,
            device=self.device,
        )
        xte = torch.zeros(num_elements, dtype=torch.float64, device=self.device)
        abs_error_sum = torch.zeros((), dtype=torch.float64, device=self.device)
        sq_error_sum = torch.zeros((), dtype=torch.float64, device=self.device)
        structure_count = torch.zeros((), dtype=torch.float64, device=self.device)
        ...
        try:
            for batch in self.train_loader:
                b = self._unpack(batch)
                pos, A, batch_idx = b["pos"], b["A"], b["batch_idx"]
                edge_src, edge_dst = b["edge_src"], b["edge_dst"]
                edge_shifts, cell = b["edge_shifts"], b["cell"]
                target_energies = b["target_energies"].view(-1)
                extras = b["extras"]
                atom_mask = None
                if isinstance(extras, dict) and torch.is_tensor(
                    extras.get("atom_mask", None)
                ):
                    atom_mask = extras["atom_mask"].to(
                        device=self.device, dtype=pos.dtype
                    ).view(-1)
                if atom_mask is None:
                    atom_mask = torch.ones_like(batch_idx, dtype=pos.dtype)

                dispersion_edges = self._dispersion_edges_for_batch(
                    pos, batch_idx, cell, extras
                )
                out = self.model(
                    pos,
                    A,
                ...
                energy_per_atom = out[0] if isinstance(out, tuple) else out
                energy_per_atom = energy_per_atom.view(-1) * atom_mask
                num_structures = int(target_energies.numel())
                predicted = torch.zeros(
                    num_structures,
                    dtype=energy_per_atom.dtype,
                    device=self.device,
                )
                predicted.index_add_(0, batch_idx, energy_per_atom)
                e0 = map_tensor_values(
                    A.to(dtype=self.values.dtype), self.keys, self.values
                ).view(-1)
                predicted.index_add_(0, batch_idx, e0 * atom_mask)
                num_atoms = torch.zeros_like(predicted)
                num_atoms.index_add_(0, batch_idx, atom_mask)
                num_atoms = num_atoms.clamp_min(1.0)
                error = (predicted - target_energies) / num_atoms

                map_index = A.clamp(
                    min=0, max=int(atomic_number_to_index.numel()) - 1
                )
                compact_idx = atomic_number_to_index[map_index]
                valid = (compact_idx >= 0).to(dtype=pos.dtype) * atom_mask
                compact_idx = compact_idx.clamp(min=0)
                composition = torch.zeros(
                    (num_structures, num_elements),
                    dtype=pos.dtype,
                    device=self.device,
                )
                for element_idx in range(num_elements):
                    counts = valid * (compact_idx == element_idx).to(pos.dtype)
                    composition[:, element_idx].index_add_(
                        0, batch_idx, counts
                    )
                composition = composition / num_atoms.unsqueeze(-1)

                x64 = composition.to(torch.float64)
                e64 = error.to(torch.float64)
                xtx.add_(x64.transpose(0, 1) @ x64)
                xte.add_(x64.transpose(0, 1) @ e64)
                abs_error_sum.add_(e64.abs().sum())
                sq_error_sum.add_(e64.square().sum())
                structure_count.add_(float(e64.numel()))
        finally:
        ...
        if int(structure_count.item()) <= 0:
            raise RuntimeError("cannot calibrate element energies on an empty train split")

        delta = -(torch.linalg.pinv(xtx, hermitian=True) @ xte)
        old = correction.detach().to(torch.float64).clone()
        correction.add_(delta.to(dtype=correction.dtype))
        new = correction.detach().to(torch.float64).clone()
        report = {
            "num_structures": int(structure_count.item()),
            "raw_train_energy_mae_per_atom": float(
                (abs_error_sum / structure_count).item()
            ),
            "raw_train_energy_rmse_per_atom": float(
                torch.sqrt(sq_error_sum / structure_count).item()
            ),
            "delta": delta.detach().cpu().tolist(),
            "old_correction": old.detach().cpu().tolist(),
            "new_correction": new.detach().cpu().tolist(),
            "normal_matrix_rank": int(
                torch.linalg.matrix_rank(xtx).detach().cpu().item()
            ),
        }
        self.element_energy_calibration_report = report
        log.info("closed-form train-only element calibration: %s", report)
        return report
    ...
    def _collect_arch_metadata(self):
        ...
        if cfg is not None:
            ...
            for attr in (
                ...
                "ictd_fix_phase_coefficient", "ictd_fix_phase_context",
                "ictd_fix_phase_placement", "ictd_fix_phase_density_rank",
                "ictd_fix_phase_density_pairs", "ictd_fix_phase_coherence_init",
                "ictd_fix_phase_normalization",
                "ictd_fix_phase_scope",
                "ictd_fix_phase_heads",
                ...
                "dispersion_min_cutoff", "dispersion_switch_width",
    ...
    def _training_metadata(self):
        return {
            ...
            "phase_rank_orthogonal_weight": self.phase_rank_orthogonal_weight,
            ...
            "element_energy_calibration": self.element_energy_calibration_report,
