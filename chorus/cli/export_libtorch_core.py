...
def _set_ictd_internal_compute_dtype_for_export(model: torch.nn.Module, dtype: torch.dtype) -> None:
    """Force ICTC-family internal TP compute dtype for deployment export only.
        ...
        for attr in cache_attrs:
            if hasattr(module, attr):
                ...
                if isinstance(cache, dict):
                    cache.clear()
    if touched:
        print(f"[export_core] forced ICTC internal_compute_dtype={str(dtype).replace('torch.', '')} on {touched} modules")
...
def _trace_model_for_export(
    ...
    *,
    ...
    from chorus.interfaces.lammps_mliap import (
...
def _export_single_core(
    *,
    ...
    dispersion_module = getattr(metadata_model, "dispersion", None)
    mbd_source_enabled = bool(getattr(metadata_model, "long_range_mbd_source_enabled", False))
    long_range_mbd_beta = float(dispersion_module.mbd_beta()) if (
        mbd_source_enabled and dispersion_module is not None and hasattr(dispersion_module, "mbd_beta")
    ) else 1.0
    long_range_mbd_coupling_scale = float(dispersion_module.mbd_coupling_scale()) if (
        mbd_source_enabled and dispersion_module is not None and hasattr(dispersion_module, "mbd_coupling_scale")
    ) else 1.0
    long_range_mbd_energy_scale = float(dispersion_module.mbd_energy_scale()) if (
        mbd_source_enabled and dispersion_module is not None and hasattr(dispersion_module, "mbd_energy_scale")
    ) else 1.0
    ...
    meta = {
        ...
        "dispersion_min_cutoff": float(
            getattr(metadata_model, "dispersion_min_cutoff", getattr(dispersion_module, "min_cutoff", 0.0))
        ),
        "dispersion_switch_width": float(
            getattr(metadata_model, "dispersion_switch_width", getattr(dispersion_module, "switch_width", 0.5))
        ),
        ...
        "long_range_mbd_source_offset": int(getattr(metadata_model, "long_range_mbd_source_offset", 0)),
        "long_range_mbd_source_channels": int(getattr(metadata_model, "long_range_mbd_source_channels", 2)),
        "long_range_mbd_beta": long_range_mbd_beta,
        "long_range_mbd_coupling_scale": long_range_mbd_coupling_scale,
        "long_range_mbd_energy_scale": long_range_mbd_energy_scale,
