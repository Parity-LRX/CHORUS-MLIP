...
void MFFTorchEngine::load_single_core_file(const std::string& core_pt_path) {
  ...
  if (core_pt_path.size() >= 4 &&
    ...
    dispersion_min_cutoff_ = 0.0;
    dispersion_switch_width_ = 0.5;
    ...
    // deployed energy would be missing the long-range term. Member defaults (header) serve as init.
    {
      ...
      if (jin) {
        ...
        (void)parse_double_from_metadata(content, "\"dispersion_min_cutoff\"", dispersion_min_cutoff_);
        (void)parse_double_from_metadata(content, "\"dispersion_switch_width\"", dispersion_switch_width_);
        ...
        (void)parse_double_from_metadata(content, "\"long_range_mbd_energy_scale\"", long_range_mbd_energy_scale_);
