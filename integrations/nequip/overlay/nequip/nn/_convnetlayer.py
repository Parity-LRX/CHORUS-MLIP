...
class ConvNetLayer(GraphModuleMixin, torch.nn.Module):
    ...
    def __init__(
        ...
        convolution_kwargs = dict(convolution_kwargs)
        chorus_scope = str(chorus_scope).lower()
        if chorus_scope not in ("final", "all", "persistent"):
            raise ValueError(
                "chorus_scope must be 'final', 'all', or 'persistent', "
        ...
        if (
            ...
            bool(convolution_kwargs.get("chorus_enabled", False))
            and chorus_scope == "persistent"
        ):
            convolution_kwargs["chorus_persistent"] = True
            convolution_kwargs["chorus_update_enabled"] = (
                int(layer_index) > 0
            )
            convolution_kwargs["chorus_clear_state"] = (
                int(layer_index) == int(num_layers) - 1
            )
