...
- `chorus/models/`: the `PureCartesianICTDFix` model, ICTC irreps, tensor product helpers, MACE-compatible symmetric contractions, radial basis functions, optional ZBL and long-range modules.
- `chorus/training/`: the energy/force/stress trainer and the `make_fx` + Inductor force-step compiler.
- `chorus/data/`: extended-XYZ parsing, H5 dataset loading, graph padding, bucket sampling, and collate functions.
- `chorus/cli/`: command-line tools for training, MACE conversion, AOTInductor export, TorchScript export, and LAMMPS helper generation.
- `chorus/interfaces/`: checkpoint loading, LAMMPS MLIAP wrappers, and deployment-facing compatibility code.
- `chorus/evaluation/`: ASE calculator wrappers.
- `chorus/bench/`: benchmark harnesses comparing MACE-ICTC modes against native `mace-torch`.
- `chorus/test/`: numerical and smoke tests.
...
|---|---|---|
| `mff-convert-mace` | `chorus.cli.convert_mace` | Convert a native `mace-torch` `ScaleShiftMACE` checkpoint to MACE-ICTC. |
| `mff-export-aoti` | `chorus.cli.export_aoti_core` | Export an AOTInductor `.pt2` core for Python/C++/LAMMPS deployment. |
| `mff-export-core` | `chorus.cli.export_libtorch_core` | Export a TorchScript core. Mostly for legacy LibTorch deployment. |
| `mff-lammps` | `chorus.cli.lammps_interface` | Generate helper files for LAMMPS-style deployment. |
...
or the lower-level helpers in `chorus.mace_basis`.
...
The parser in `chorus.data.preprocessing` supports extended XYZ-style data with:
...
The dataset loader is `chorus.data.datasets.H5Dataset`; batching uses `chorus.data.collate.collate_fn_h5`.
...
fresh-build converter parity test (`chorus/test/test_mace_converter.py`) builds its MACE in-process
...
The ASE wrapper is `chorus.evaluation.calculator.MyE3NNCalculator`.
...
|---|---|---|
| Tool | `chorus.cli.export_aoti_core` | `chorus.cli.export_libtorch_core` |
...
| File | Why it matters |
|---|---|
| `chorus/models/pure_cartesian_ictd_fix.py` | Main model and product backends. |
| `chorus/mace_basis.py` | Orthogonal ICTC/e3nn basis conversion. |
| `chorus/interfaces/mace_converter.py` | Native MACE to MACE-ICTC weight conversion. |
| `chorus/cli/export_aoti_core.py` | AOTI export, cuEq product replacement, angular-basis export logic. |
| `chorus/training/makefx_compile.py` | `make_fx` force-step compilation. |
| `chorus/training/train_loop.py` | Trainer, checkpoint metadata, ScaleShift/E0 loss handling. |
| `chorus/interfaces/lammps_mliap.py` | Deployment checkpoint reload and wrapper logic. |
| `chorus/test/test_mace_converter.py` | Native MACE to MACE-ICTC conversion parity tests. |
