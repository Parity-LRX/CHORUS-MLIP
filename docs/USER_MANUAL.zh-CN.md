...
本文档说明 MACE-ICTC 代码库的整体结构、常用模式、训练、MACE 原生模型转换、AOTInductor 导出、LAMMPS 部署、benchmark 和测试方法。

English version: [USER_MANUAL.md](USER_MANUAL.md)

## 1. 这个仓库包含什么

MACE-ICTC 是一个独立的 MACE 实现：它把 MACE 的等变特征从原生 e3nn 球谐基表示改写到 ICTC（Irreducible Cartesian Tensor Decomposition，不可约笛卡尔张量分解）基中，并带有训练、转换、导出和 LAMMPS 部署栈。

核心技术点是 ICTC basis construction：用固定的 `Q`/`U` 算子把 MACE/e3nn 的角向代数重写成不可约笛卡尔张量块，同时保留原始 MACE 的 interaction/readout 语义。这个仓库重点解决 basis conversion、原生 `mace-torch` 数值对应和部署。

主要目录：

- `chorus/models/`：`PureCartesianICTDFix` 主模型、ICTC irreps、tensor product、MACE-compatible symmetric contraction、radial basis、ZBL、long-range 等。
- `chorus/training/`：energy/force/stress trainer，以及 `make_fx` + Inductor 的 force step 编译。
- `chorus/data/`：extended XYZ 解析、H5 dataset、padding、bucket sampler、collate。
- `chorus/cli/`：训练、MACE 转换、AOTI 导出、TorchScript 导出、LAMMPS helper。
- `chorus/interfaces/`：checkpoint 加载、LAMMPS MLIAP wrapper、部署兼容逻辑。
- `chorus/evaluation/`：ASE calculator。
- `chorus/bench/`：MACE-ICTC 和原生 `mace-torch` 的 benchmark。
- `chorus/test/`：数值一致性和 smoke tests。
...
可选 ICTC tensor-product C++/CUDA extension：
...
| 命令 | Python 入口 | 用途 |
|---|---|---|
| `mff-convert-mace` | `chorus.cli.convert_mace` | 把原生 `mace-torch` `ScaleShiftMACE` 转成 MACE-ICTC checkpoint。 |
| `mff-export-aoti` | `chorus.cli.export_aoti_core` | 导出 AOTInductor `.pt2` core，用于 Python/C++/LAMMPS 部署。 |
| `mff-export-core` | `chorus.cli.export_libtorch_core` | 导出 TorchScript core，主要用于旧的 LibTorch 部署。 |
| `mff-lammps` | `chorus.cli.lammps_interface` | 生成 LAMMPS 部署 helper。 |

也可以直接用 module 方式：

```bash
python -m chorus.cli.train --help
python -m chorus.cli.convert_mace --help
python -m chorus.cli.export_aoti_core --help
```

## 4. 核心概念

### 4.1 ICTC 基和 e3nn/MACE 基

原生 MACE 使用 e3nn 球谐基。MACE-ICTC 在内部把等变特征存成 ICTC 笛卡尔基。两者之间由固定的正交矩阵 `Q_l` 相连。
...
或者直接使用 `chorus.mace_basis` 里的底层函数。
...
| 值 | 含义 | 适用场景 |
|---|---|---|
| `ictd` | 默认 ICTC 内部基。 | parity 基准、bridge-U、最保守路径。 |
...
| Backend | 含义 | 推荐用途 |
|---|---|---|
| `ictd-bridge-u` | 使用 MACE/e3nn symmetric-contraction U，并把 ICTC/e3nn basis bridge fold 到 U 里。 | MACE parity、原生 MACE 转换、高 `max_ell` 稳妥路径。 |
...
| `ictd-pure-u` | 直接用 ICTC 生成的 U。 | 诊断路径，不是当前高 `max_ell` 主生产路径。 |
...
- 从头训练 MACE-ICTC 时，只有在你明确要 reduced-CG 架构时才开。
...
| 从 ICTC checkpoint 导出高吞吐 AOTI | `mff-export-aoti --cueq-product --angular-basis e3nn`，前提是部署环境能加载 cuEq custom op。 |
...
MACE-ICTC 训练 CLI 支持 PyTorch `DistributedDataParallel`。默认 `--ddp auto`
...
srun python -m chorus.cli.train \
  --data-dir DATA \
  --train-prefix train \
  --val-prefix val \
  --product-backend cueq \
...
`chorus.data.preprocessing` 支持 extended XYZ 风格数据：
...
```python
from chorus.data.preprocessing import save_to_h5_parallel
...
训练 dataset 是 `chorus.data.datasets.H5Dataset`，batching 使用 `chorus.data.collate.collate_fn_h5`。
...
MACE-ICTC 里用 `--long-range-mode reciprocal-spectral-v1` 训练或 fine-tune。
...
- `ictd-bridge-u`：推荐转换 backend；通过 ICTC/e3nn basis bridge fold MACE/e3nn 的 U convention，是主数值对应路径。
...
- `ictd-pure-u`：直接用 ICTC 生成 U 的诊断路径，不是原生 MACE exact conversion 主路径。

converter 会读取 `mace_model.use_reduced_cg` 并用同样配置重建 MACE-ICTC。用户转换原生 MACE 模型时不应该手动猜这个选项。
...
`mace-torch`/`e3nn` 环境来完成 load 和 conversion；转换后的 ICTC checkpoint 可以再交给当前
MACE-ICTC runtime 加载、导出和部署。
...
# 前置它们来 load + convert（mace_ictc 也要可导入：PYTHONPATH 或已安装）：
PYTHONPATH=$HOME/compat_e3nn044/mace_torch_0_3_16:$HOME/compat_e3nn044/e3nn_0_4_4 \
  python -m chorus.cli.convert_mace \
...
（`chorus/test/test_mace_converter.py`）在进程内现建 MACE，不受影响；这个 legacy 环境只用于加载*已存档的*
...
ASE wrapper 是 `chorus.evaluation.calculator.MyE3NNCalculator`。
...
from chorus.evaluation.calculator import MyE3NNCalculator
...
MACE-ICTC 学习两类长程修正，都是网络内置的（不依赖外部 Ewald 或 libMBD），都能通过
...
- **极化率阶数** `--mbd-anisotropic`。关 = 各向同性标量 α（发射 `[N,2]` 源 `(ω, α)`）；开 =
  **各向异性 l=2 张量** α（发射 `[N,8]` `(ω, α_iso, 6×B)`，耦合 W=ω·B）。张量从 l=2 节点块构造，
  需要 `--lmax ≥ 2`。这是 ICTC 独有的路径，且几乎免费（见 12.5）。
...
```bash
python -m chorus.cli.train ... --long-range-dispersion-mode pairwise-c6 --dispersion-cutoff 8.0 --checkpoint model_c6.pth
```
...
|---|---|---|
| 工具 | `chorus.cli.export_aoti_core` | `chorus.cli.export_libtorch_core` |
...
```bash
python -m chorus.cli.export_aoti_core --checkpoint model_mbd_aniso.pth --out model.pt2
```
...
```bash
python -m chorus.cli.export_aoti_core --route baseline --channels 128 --lmax 2 \
...
```bash
# (a) MBD 色散
python -m chorus.cli.train --data-dir DATA \
  --channels 128 --lmax 2 --num-interaction 2 \
...
```bash
python -m chorus.bench.synthetic_workloads \
  --device cuda \
  --dtype float32 \
  --channels 64 \
  ...
  --train-iters 5 \
  --infer-iters 20 \
  --out-dir /tmp/chorus_synthetic_bench
```
...
```bash
python -m chorus.test.test_training_smoke
python -m pytest chorus/test/test_angular_basis.py -q
python -m pytest chorus/test/test_export_aoti_core.py -q
```

MACE converter 验证：

```bash
python -m chorus.test.test_mace_converter
```

cuEq product 测试：

```bash
python -m pytest chorus/test/test_cueq_product_backend.py -q
python -m chorus.test.test_cueq_makefx_training
```
...
从 bridge-U product 替换到 cuEq product 时，只能拷 learnable MACE contraction weights，不能拷 bridge-U 的固定 `U_matrix_*` buffer。bridge-U 的 U 已经包含 ICTC/e3nn basis fold，直接拷到 cuEq 会错。当前 export path 已处理这个问题。
...
- smoke test 通过不等于 MACE parity 已证明；parity 要跑 MACE-vs-ICTC converter test。
...
| 文件 | 作用 |
|---|---|
| `chorus/models/pure_cartesian_ictd_fix.py` | 主模型和 product backends。 |
| `chorus/mace_basis.py` | ICTC/e3nn 正交 basis 转换。 |
| `chorus/interfaces/mace_converter.py` | 原生 MACE 到 MACE-ICTC 的权重转换。 |
| `chorus/cli/export_aoti_core.py` | AOTI 导出、cuEq product replacement、export-time angular basis 逻辑。 |
| `chorus/training/makefx_compile.py` | `make_fx` force-step 编译。 |
| `chorus/training/train_loop.py` | Trainer、checkpoint metadata、ScaleShift/E0 loss 逻辑。 |
| `chorus/interfaces/lammps_mliap.py` | 部署端 checkpoint reload 和 wrapper 逻辑。 |
| `chorus/test/test_mace_converter.py` | 原生 MACE 到 MACE-ICTC 的转换 parity 测试。 |
